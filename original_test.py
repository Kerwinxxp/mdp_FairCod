...
agent = torch.load('model/agent_wtnoise.pth')
replayBuffer = ReplayBuffer(memorySize, batchSize)




dayRecorder = open('info/day_reward.txt', 'a+')   # a+不会被覆写
...
while dayIndex < maxDay:  # 利用30天的订单
    ...
    while T < maxTime:  # 这一天的时间
        ...
        for order in env.dayOrder[env.cityTime]: # 输入这个时段的订单
            ...
            # 订单订单状态
            orderStateArray = env.order_state_compute(courierList, order)  # n * k(订单特征)  # 7
            # 计算系统整体的供需关系
            ...
            for i in range(orderStateArray.shape[0]):
                ...
                tempSymbol = orderStateArray[i][6]
